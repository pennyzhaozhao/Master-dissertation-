...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train KNN on HOG features from balanced splits.")
...
def main() -> None:
    ...
    for k in [3, 5, 7, 9, 11]:
        ...
        validation_results.append({"k": k, "validation_f1": float(val_f1)})
        print(f"K={k}: validation F1={val_f1:.4f}")
        if val_f1 > best_f1:
            best_f1 = val_f1
            best_k = k

    print(f"Best K: {best_k} with validation F1={best_f1:.4f}")

    x_train_val_scaled = np.vstack([x_train_scaled, x_val_scaled])
    y_train_val = np.concatenate([y_train, y_val])
    x_test_scaled = scaler.transform(x_test)

    final_model = KNeighborsClassifier(n_neighbors=int(best_k), n_jobs=-1)
    ...
    joblib.dump(final_model, models_dir / "knn_hog_model.joblib")
    joblib.dump(scaler, models_dir / "knn_hog_scaler.joblib")
    save_plots(y_test, y_pred, y_score, plots_dir, "knn_hog")
    ...
    with (metrics_dir / "knn_hog_metrics.json").open("w", encoding="utf-8") as f:
